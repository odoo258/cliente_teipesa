...
# LGD esto esta hardcodeado debemos cambiar todo este codigo ASAP!!!!
...
def to_word(number, mi_moneda=None):
    ...
    if num_decimals != ['00']:
        sentence = sentence + ' con ' + ' '.join(human_readable_decimals).replace('  ', ' ').title().strip() + ' Centavos.'
    if num_decimals == ['00']:
        sentence = sentence + ' con Cero Centavos.'
    return sentence
