...
class AccountMoveLine(models.Model):
    ...
    def compute_amount_fields_custom(self,obj, amount, src_currency, company_currency, invoice_currency=False):
        ...
        if src_currency and src_currency != company_currency:
            ...
            if obj.manual_currency_rate_active:
                if obj.manual_rate_option == 'rate':
                    amount = amount_currency / obj.manual_currency_rate
                else:
                    amount = amount_currency * obj.inverse_currency_rate
		    print "=============amount_currency===",amount_currency,obj.manual_currency_rate,amount_currency * obj.manual_currency_rate
        ...
        if invoice_currency and invoice_currency != company_currency and not amount_currency:
            if obj.manual_currency_rate_active:
                if obj.manual_rate_option == 'rate':
                    ...
                    amount_currency = amount * obj.inverse_currency_rate
            currency_id = invoice_currency.id
	print "=======debit, credit, amount_currency, currency_id============",debit, credit, amount_currency, currency_id
    ...
    def _compute_total_invoices_amount(self):
        ...
        if all(inv.currency_id == payment_currency for inv in invoices):
            ...
            if not self.manual_currency_rate_active:
                ...
                total = 0
                for inv in invoices:
                    if self.manual_rate_option == 'rate':
                        total += inv.residual * self.manual_currency_rate
                    else:
			print "=============inv.residual / self.inverse_currency_rate=",inv.residual / self.inverse_currency_rate,self.inverse_currency_rate
                        total += inv.residual / self.inverse_currency_rate
        return abs(total)
...
class account_payment(models.Model):
    ...
    @api.one
    @api.depends('invoice_ids', 'amount', 'payment_date', 'currency_id','manual_currency_rate','inverse_currency_rate')
    ...
    def _create_payment_entry(self, amount):
	    ...
	    print "==================self,amount, self.currency_id, self.company_id.currency_id, invoice_currency",self,amount, self.currency_id, self.company_id.currency_id, invoice_currency
