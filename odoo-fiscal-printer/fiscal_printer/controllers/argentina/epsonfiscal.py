...
class DummyDriver:
    ...
    def __init__(self):
        try:
            self.number = int(raw_input("Ingrese el nï¿½mero de la ï¿½ltima factura: "))
        except EOFError:
...
#    print "REG ::: ",str(self.getRegisteredTaxNumber()) 	
    ...
    def _openBillCreditTicket(self, type, name, address, doc, docType, ivaType, isCreditNote,
        ...
        if self.model == "epsonlx300+":
            parameters = [isCreditNote and "N" or "F", # Por ahora no soporto ND, que serï¿½a "D"
                "C",
                type, # Tipo de FC (A/B/C)
                "1",   # Copias - Ignorado
                "P",   # "P" la impresora imprime la lineas(hoja en blanco) o "F" preimpreso
                "17",   # Tamaï¿½o Carac - Ignorado
            ...
            parameters = [isCreditNote and "M" or "T", # Ticket NC o Factura
               ...
               # self.ivaTypeMap.get(ivaType, "F"),   # Iva Comprador
                ...
                ("%s"%str(self.getDocumentCode())) or formatText(docType) or (isCreditNote and "-" or ""),
                 # Tipo de Doc. - Si es NC obligado pongo algo
                ("%s"%str(self.getRegisteredTaxNumber())) or doc or (isCreditNote and "-" or ""), # Nro Doc - Si es NC obligado pongo algo
...
############################################################################################################
    ...
    def addToTotal(self, description, quantity, price, iva, discount, discountDescription, negative=False):
        ...
        extraparams = self._currentDocument in (self.CURRENT_DOC_BILL_TICKET, self.CURRENT_DOC_CREDIT_TICKET) and ["", "", ""] or []
        print "GET COMMAND INDEX :: ", self._getCommandIndex()
        if self._getCommandIndex() == 0:
            for d in description[:-1]:
                self._sendCommand(self.CMD_PRINT_TEXT_IN_FISCAL, [formatText(d)[:20]])
        reply = self._sendCommand(self.CMD_PRINT_LINE_ITEM[self._getCommandIndex()], [formatText(description[-1][:20]), quantityStr, priceUnitStr, ivaStr, sign,bultosStr,"0" * 8]+extraparams)

    def SubToTotal(self, description, quantity, price, iva, discount, discountDescription, negative=False):
        if type(description) in types.StringTypes:
            description = [description]
        sign = 'R'
        quantityStr = str(int(quantity * 1000))
...
#             print "priceUnitStr :: ",priceUnitStr
     ...
     #        discountdiff =
            discount = '%0.2f' % (discount)
            # discount = (discount)
            # discountdiff= round(discount*100,2)
            # discountStr = str(int(discountdiff))
            discountStr = str(int(round(float(discount)*100,0)))
...
#                 return reply
    ...
    def getLastNumber(self, letter):
        ...
        if len(reply) < 3:
# La respuesta no es vï¿½lida. Vuelvo a hacer el pedido y si hay algï¿½n error que se reporte como excepciï¿½n
