...
class FiscalPrinterPoroxy(http.Controller):
    @http.route('/hw_proxy/fiscal_printer_print_xml_backend_receipt', type='json', auth='none',
                cors='*')  # For Backed End Model Receipt Print
    def fiscal_printer_print_xml_backend_receipt(self, receipt, printer_config, printer_port, printer_model,
                                                 account_invoice=False):
        if receipt:
            mp = self.backendReceiptPrintConnection(receipt, printer_config, printer_port, printer_model,
                                                    account_invoice)
    ...
    def backendReceiptPrintConnection(self, receipt, printer_config, printer_port, printer_model,
                                      account_invoice):  # For Backed End Model Receipt Print
        if printer_port:
            if printer_model:
                model = printer_model
    ...
    def PrinterToPrintBackenedReceipt(self, printer, model, receipt,
                                      account_invoice):  # For Backed End Model Receipt Print
            ...
            for data in receipt.get('product'):
                if type(data) is dict:
                    ...
                    if receipt.get('receipt_type') == 'A':  # RECEIPT TYPE 'A'
                        ...
                        # else:
                        #     price = float(
                        #         str(price).split(".")[0] + "." + "00")  # "%0.2s" %(str(price).split(".")[-1]))
                        ...
                        try:
                            if tax:
                                ...
                                print 'VAT PRICE : ',str(ivaprice)

                                # if len(str(float(ivaprice)).split(".")[-1]) == 2 or len(
                                #         str(float(ivaprice)).split(".")[-1]) == 1:  # VAT PRICE CALCULATION
                                #     if (str(float(ivaprice)).split(".")[-1]) == '0':
                                #         price = float(str(ivaprice).split(".")[0] + "." + "00")
                                #         print '0 PRICE UNIT :------------------------------', price
                                #     else:
                                #         ivaprice = float(str(ivaprice).split(".")[0] + "." + "%0.2s" % (
                                #         str(ivaprice).split(".")[-1]))
                                ...
                                #     priceUnit = float(
                                #         str(price).split(".")[0] + "." + "%0.3s" % (str(price).split(".")[-1]))
                                #     priceRemain = "0.00" + str(priceUnit)[-1]
                                #     priceQuantity = float(priceRemain) * quantity
                                #     print 'priceQuantity :: -------------------------------------', priceQuantity
                                #     productMake.append(priceQuantity)
                                #     ivaprice = float(str(ivaprice).split(".")[0] + "." + "%0.2s" % (
                                #     str(ivaprice).split(".")[-1]))  # PRICE UNIT STR
                                #     print '2 priceUnitStr  : ---------------------', ivaprice
                                # else:
                                #     ivaprice = float(str(ivaprice).split(".")[
                                #                          0] + "." + "00")  # "%0.2s" %(str(ivaprice).split(".")[-1]))
                                #     print '3 priceUnitStr  : ---------------------', ivaprice

                                if discount > 0:  # DISCOUNT CALCULATION
                                    discountstr = (discount * float(ivaprice) * quantity) / 100.00
                                    discountDescription = "%s" % discount + "%" + " Discount "
                                    printer.addItem(product, quantity, ivaprice, tax, discountstr, discountDescription)
                                elif discount < 0:  # SURCHARGE CALCULATION
                                    discountstr = (
                                    price * (float(discount) / 100.00) * quantity * ((float(tax) / 100.00) + 1))
                                    discountDescription = "%s" % str(abs(discount)) + "%" + " Surcharge "
                                    printer.addItem(product, quantity, ivaprice, tax, discountstr, discountDescription)
                                elif discount == 0:
                                ...
                                ivaprice = float("%0.2f" % (ivaprice))
                                print 'VAT PRICES(FLOAT) : ',str(ivaprice)
                                # if len(str(float(ivaprice)).split(".")[-1]) == 2 or len(
                                #         str(float(ivaprice)).split(".")[-1]) == 1:
                                #     if (str(float(ivaprice)).split(".")[-1]) == '0':
                                #         price = float(str(ivaprice).split(".")[0] + "." + "00")
                                #         print '0 PRICE UNIT :------------------------------', price
                                #     else:
                    ...
                    if type(percept) is dict:
                        if percept.get("receipttype") == "perception":
                            if receipt.get("receipt_type") == 'A':
                                printer.AddPerception(percept.get("percpt_text"), percept.get("percpt_amount"),
                                                      percept.get("percpt_tax"))
                                ...
                                discountstr = 0
                                discountDescription = ''
                                printer.addItem(percept.get("percpt_text"), quantity, percept.get("percpt_amount"),
                                                percept.get("percpt_tax"), discountstr, discountDescription)
            except:
                pass
    ...
    def fiscal_printer_print_xml_receipt(self, receipt, printer_config, printer_port, printer_model,
                                         partner_afip_code=False, partner_code=False, id_number=False,
                                         customer_name=False, customer_address=False, main_id_number=False,
                                         afip_responsability_type_id=False):  # For pos front end receipt print
        if receipt:
            mp = self.printconnection(receipt, printer_config, printer_port, printer_model, partner_afip_code,
                                      partner_code, id_number, customer_name, customer_address, main_id_number,
    ...
    def printconnection(self, receipt, printer_config, printer_port, printer_model, partner_afip_code, partner_code,
                        id_number, customer_name, customer_address, main_id_number, afip_responsability_type_id):
        if printer_port:
            if printer_model:
                model = printer_model
                if printer_config == 'panama':
                    ...
                    printer = EpsonPrinter(deviceFile=printer_port, model=printer_model, dummy=False)
                    pool = ThreadPool(processes=1)
                    async_result = pool.apply_async(self.PrinterToPrint, args=(
                    printer, model, receipt, partner_afip_code, partner_code, id_number, customer_name,
                    customer_address, main_id_number, afip_responsability_type_id))
                    return async_result.get()
    ...
    def PrinterToPrint(self, printer, model, receipt, partner_afip_code, partner_code, id_number, customer_name,
                       customer_address, main_id_number, afip_responsability_type_id):
        ...
        if type(receipt) is dict and receipt.get('report') == 'autocancelreceipt':
            ...
            for taxs in taxDetails:
                ...
                for perceptTax in perceptionTaxs:
                    if taxname == perceptTax:
                        perception.append({'tax': taxs.get('amount'), 'name': taxs['tax']['name'],
                                           'taxamount': taxs['tax']['amount']})
            ...
            for products in receipt['receipt']['orderlines']:
                ...
                if tax:
                    ...
                    # else:
                    #     price = float(
                    #         str(price).split(".")[0] + "." + "00")  # "%0.2s" %(str(price_org).split(".")[-1]))
                    #     print '3 priceUnitStr  : ------------------------------------------', price
                ...
                try:
                    if ResponsabilityType == 'I':  # FOR THE RECEIPT TYPE 'A'
                        # if len(str(float(price_org)).split(".")[-1]) == 2 or len(
                        #         str(float(price_org)).split(".")[-1]) == 1:
                        #     if (str(float(price_org)).split(".")[-1]) == '0':
                        #         price = float(str(price_org).split(".")[0] + "." + "00")
                        #         print '0 PRICE UNIT :------------------------------', price
                        #     else:
                        #         price = float(
                        #             str(price_org).split(".")[0] + "." + "%0.2s" % (str(price_org).split(".")[-1]))
                        #         print '1 priceUnitStr  : ------------------------------------------', price
                        # elif len(str(float(price_org)).split(".")[-1]) > 2:
                        #     priceUnit = float(
                        #         str(price_org).split(".")[0] + "." + "%0.3s" % (str(price_org).split(".")[-1]))
                        #     priceRemain = "0.00" + str(priceUnit)[-1]
                        #     priceQuantity = float(priceRemain) * quantity
                        #     print 'priceQuantity :: ------------------------------------------', priceQuantity
                        #     productMake.append(priceQuantity)
                        ...
                        # else:
                        #     price = float(
                        #         str(price_org).split(".")[0] + "." + "00")  # "%0.2s" %(str(price_org).split(".")[-1]))
                        #     print '3 priceUnitStr  : ------------------------------------------', price
            ...
            try:
                if perception:
                    for perceptTax in perception:
                        printer.AddPerception(perceptTax.get("name"), perceptTax.get("tax"),
                                              perceptTax.get("taxamount"))
            except:
                pass
