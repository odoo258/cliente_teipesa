...
class FiscalPrinterPoroxy(http.Controller):
    ...
    def PrinterToPrintBackenedReceipt(self, printer, model, receipt,account_invoice):  # For Backed End Model Receipt Print
        ...
        if type(receipt) is dict and receipt.get('report') == 'autocancelreceipt':
            ...
            if receipt.get('receipt_type') == 'B':
            ...
            elif receipt.get('receipt_type') == 'A':
                listTypeA=[]
                for data in receipt.get('product'):
                    if type(data) is dict:
                        tax = data.get('tax') or 0
                        ...
                        try:
                            if discount > 0:  # DISCOUNT CALCULATION
                                ...
                                priceProduct =float("%0.2f"% (price * quantity))
                                totals =float("%0.2f" %(priceProduct - discountstr))
                                if tax:
                                    listTypeA.append({'tax':tax,'price':totals})
                                else:
                                    receiptA += float(totals)
                                ...
                                priceProduct =float("%0.2f"% (price * quantity))
                                if tax:
                                    listTypeA.append({'tax':tax,'price':priceProduct})
                                else:
                                    receiptA += float(priceProduct)
                        except:
                            pass
                ...
                try:
                    if listTypeA:
                        ...
                        for data in listTypeA:
                            taxList.append(data.get('tax'))
                        if taxList:
                            for datas in list(set(taxList)):
                                lstlist.append({'tax': datas,'price': []})

                        for data in listTypeA:
                            for lst in lstlist:
                                if data.get('tax') == lst.get('tax'):
                                    lst.get('price').append(data.get('price'))
                        ...
                        if lstlist:
                            ...
                            for data in lstlist:
                                sum = 0
                                # total = 0
                                for dat in data.get('price'):
                                    sum = sum + dat

                                receiptA += sum
                                receiptA += float("%0.2f" % (sum * data.get('tax') / 100.00))
                                print 'Receipt A :-------------------',receiptA
                                # receiptA = float("%0.2f" %(sum * data.get('tax') / 100.00))
                                print 'taxs:----------------------', data.get('tax')
                            # print 'PRICES : ----', total
                            # receiptA += float("%0.2f" %(total))


                        # if lstlist:
                        #     print 'TAX LIST:--------------------',lstlist
                        #     total = 0
                        #     pricetotal=0
                        #     for data in lstlist:
                        #         if data.get('tax'):
                        #             for pricelist in data.get('price'):
                        #                 pricetotal += pricelist
                        #             total += float(data.get('tax') / 100 + 1) * float(pricetotal)
                        #     receiptA+=total
                ...
                try:  # PERCEPTION
                    for percept in receipt.get("perception"):
                        if type(percept) is dict:
                            if percept.get("receipttype") == "perception":
                                    receiptA += float(percept.get("percpt_amount"))
                except:
                    pass

                print 'RECEIPT TYPE A :--------------------------------------------', receiptA
                ...
                if receipt.get('product')[-1:]:
                    for data in receipt.get('product')[-1:]:
                        if type(data) is dict:
                            ...
                            quantity = data.get('quantity') or 0
                            discountDescription = ''
                            print 'RECIECPT -A :----------------------------------------------',receiptA
                            diffTotal = float(receiptA)
                            print 'diffTotal :--------------------------------------------------',diffTotal,type(diffTotal)
                            try:
                                if discount > 0:  # DISCOUNT CALCULATION
                                    # receiptA = diffTotal
                                    # discountstr = float("%0.2f" % ((discount * price * quantity) / 100.00))
                ...
                if model in models:
                    if afip_responsability_type_id:
                        ...
                        if ResponsabilityType == 'I':  # RECEIPT TYPE 'A'
                            receiptTypeA = ResponsabilityType
                            if id_number:
            ...
            if receipt_type:
                ...
                if receiptTypeA:
                    ...
                    for products in receipt['receipt']['orderlines']:
                        ...
                        try:
                            if discount > 0:
                                ...
                                priceProduct = float("%0.2f" % (price_org * quantity))
                                recDict = float("%0.2f" % (priceProduct - discountstr))
                                if tax:
                                    listTypeA.append({'tax': tax, 'price': recDict})
                                else:
                                    receiptA += float("%0.2f" % (recDict))
                                ...
                                if tax:
                                    ...
                                    receiptA += float("%0.2f" % (priceProduct))
                        except:
                            pass
                        ...
                        if listTypeA:
                            ...
                            if lstlist:
                                ...
                                for data in lstlist:
                                    ...
                                    receiptA += sum
                                    receiptA += float("%0.2f" % (sum * data.get('tax') / 100.00))
                                    # total = float("%0.2f" % (sum * data.get('tax') / 100.00))
                                    print 'TOTAL :-------------------------------',receiptA
                                    # receiptA = total
                                    print 'TOTAL :--------------------------------',receiptA
                                    print 'taxs:----------------------', data.get('tax')
                                    print 'PRICES : ----', total
                    ...
                    try:
                        if perception:
                            for perceptTax in perception:
                                receiptA += float(perceptTax.get("tax"))
                    except:
                        pass

                    print 'RECEIPT :--------------------------------------------', receiptA
                            ...
                            if discount > 0:
                                ...
                                if diffTotal and total_with_tax:
                                    ...
                                    if float(total_with_tax) > float(diffTotal):
                                        ...
                                        discountDescription = " Surcharge "
                                        printer.addItem(product, quantity, price_org, tax, outprice, discountDescription)
                                    else:
                                        outprice = float("%0.2f" % (float(diffTotal) - float(total_with_tax)))
                                        if outprice:
                                            discountDescription = " Discount "
